...
\\]
\\[
ans=\sum_d^{min(n,m)} d \sum_x^{ \lfloor \frac{n}{d} \rfloor} \sum_y^{\lfloor \frac{m}{d} \rfloor } x*y\ \ \ \ \ \ \ (gcd(x,y)=1)
\\]
...
\\]
\\[
F(n,m)=\sum_c^{min(n,m)}\mu(c)\sum_{c|x}^n\sum_{c|y}^m x*y \\
\\]
\\[
F(n,m)=\sum_c^{min(n,m)}\mu(c)*c^2 \sum_x^{\lfloor \frac{n}{c}\rfloor}\sum_y^{\lfloor \frac{m}{c} \rfloor} x*y
\\]
