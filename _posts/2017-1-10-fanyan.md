...
\\[
\begin{align} ans&=\sum_d^{min(n,m)} \sum_x^{ \lfloor \frac{n}{d} \rfloor} \sum_y^{\lfloor \frac{m}{d} \rfloor } \frac{xd*yd}{d} \\ &=\sum_d^{min(n,m)} d \sum_x^{ \lfloor \frac{n}{d} \rfloor} \sum_y^{\lfloor \frac{m}{d} \rfloor } x*y\ \ \ \ \ \ \ (gcd(x,y)=1)  \end{align}
\\]
...
\\[
\begin{align} F(n,m)&=\sum_x^n\sum_y^m(x*y*\sum_{c|gcd(x,y)}\mu(c)) \\ &=\sum_c^{min(n,m)}\mu(c)\sum_{c|x}^n\sum_{c|y}^m x*y \\ &=\sum_c^{min(n,m)}\mu(c)*c^2 \sum_x^{\lfloor \frac{n}{c}\rfloor}\sum_y^{\lfloor \frac{m}{c} \rfloor} x*y \end{align}
\\]
